...
# convert datetime datatype i.e object into datetime datatype
...
def export_data_to_big_query(data, **kwargs) -> None:
    ...
    table_id = 'uberdataanalysis-388422.uberdataset.fact_table'
    ...
    for key, value in data.items():
        table_id = 'uberdataanalysis-388422.uberdataset.{}'.format(key)  # creating the table dynamically
        BigQuery.with_config(ConfigFileLoader(config_path, config_profile)).export(
            DataFrame(value),  # converting the value in dataframe
            table_id,
            if_exists='replace'  # Specify resolution policy if table name already exists
        )